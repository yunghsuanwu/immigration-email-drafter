export function RuleChange() {
  return (
    <section id="project-intro" className="py-6 md:py-10 flex justify-center px-4 mb-8">
      <div className="max-w-2xl w-full text-left mx-auto bg-white rounded-lg shadow p-6">
        <p className="font-bold text-muted-foreground mb-4">
          The four changes that will take effect by 22 July if the parliament passes it:
        </p>
        <ul className="list-disc list-inside space-y-2">
          <li>
            raising the skills threshold for Skilled Worker visas, removing 111 eligible occupations;
          </li>
          <li>
            closing the social care worker visa route to overseas recruitment;
          </li>
          <li>
            only allowing time-limited access below degree level through a targeted immigration salary list and temporary shortage list, for critical roles only, with strict requirements for sectors to grow domestic skills;
          </li>
          <li>
            commissioning the Migration Advisory Committee (MAC) to conduct a review of the temporary shortage list including occupations, salaries and benefits.
          </li>
        </ul>
      </div>
    </section>
  )
}

export function WhyWriteToYourMP() {
  return (
    <section className="py-6 md:py-10 text-center px-4 mb-8">
      <p className="font-bold text-muted-foreground max-w-2xl mx-auto mt-4">
        With the immigration rule change imminent, this tool helps you draft a personalized, professional email to
        your Member of Parliament about your concerns regarding the immigration white paper.
        <br />
        Explain further why write to your MP.
      </p>
    </section>
  )
}